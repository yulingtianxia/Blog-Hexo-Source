...
无论是节流还是防抖，都需要设定一个时间 `durationThreshold` 阈值来限制频率，都意味着方法在最后会延迟调用。`MTRule` 默认的模式是 `MTPerformModeDebounce`，也就是防抖模式，需要等消息不再连续频繁发送后才执行。`MTPerformModeLast` 和 `MTPerformModeFirstly` 对应着节流模式，也就是控制一定时间内只执行一次。却别在于前者执行的是这段时间内最后发送的消息，后者执行第一次发送的消息。
